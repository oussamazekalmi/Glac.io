...
@section('content')
    <main class="content-fluid pt-4">
        <div class="container py-5">
            <form action="{{ route('deliveries.store') }}" method="post" class="d-flex gap-2">
                @csrf
                        ...
                        <div class="mt-4 mb-5 position-relative custom-dropdown-wrapper">
                            ...
                            <div class="custom-dropdown-toggle rounded-2 text-start" onclick="toggleDropdown()">
                                <span id="selected-client-label" class="text-secondary">
                                    {{ old('client_id') ? $clients->firstWhere('id', old('client_id'))?->full_name : 'cliquez pour choisir' }}
                                </span>
                ...
                <div class="card right-card position-relative border-0 rounded-3 shadow-sm pt-4 px-2" style="width:36%">
                    ...
                    <div class="card-body mb-3 mt-5">
                        ...
                        <div class="position-relative">
                            <label class="mb-2 ms-2 text-secondary">Quantité à livrer</label>
                            <input type="number" name="glacons_kg" class="form-control kg-input ps-3 pe-5 text-secondary border-0 rounded-2"
                                value="{{ old('glacons_kg') }}" autocomplete="off" min="1" required />
                            <span class="position-absolute text-secondary kg-span" style="font-weight: 500; right: 90%; top: 70%; transform: translateY(-50%); pointer-events: none;">kg</span>
                        </div>
